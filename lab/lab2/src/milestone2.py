...
class CrapProtocol(StackingProtocol):
    def __init__(self,mode):
        ...
        with open("/home/student_20194/.playground/connectors/crap4/team6_signed.cert", mode='rb') as f:
            signed_cert = f.read()
    ...
    def gen_cert(self):
        with open("/home/student_20194/.playground/connectors/crap4/end_key.pem", mode='rb') as f:
            end_key = f.read()
            signk = load_pem_private_key(end_key, b"passphrase", default_backend())
        
        with open("/home/student_20194/.playground/connectors/crap4/end_cert.pem", mode='rb') as f:
            end_cert = f.read()
    ...
    def data_received_server(self, packet):
        ...
        #Process4: Packet from client A has nonceSignature
            ...
            self.server_verify_nonce(self.temp_certA, packet.nonceSignature, self.nonceB)
    ...
    def client_verify_handshake(self, packet):
        ...
        self.client_verify_nonce(packet.cert, packet.nonceSignature, self.nonceA)
    ...
    def server_verify_nonce(self, cert, nonceSignature, nonce):
        client_cert = x509.load_pem_x509_certificate(cert, default_backend())
        client_cert_pubK = client_cert.public_key()
        try:
            client_cert_pubK.verify(nonceSignature, str(nonce).encode('ASCII'), padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH),hashes.SHA256())
        except Exception as e:
    ...
    def client_verify_nonce(self, cert, nonceSignature,nonce):
